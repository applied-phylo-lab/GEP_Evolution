...
class DataConfig:
    ...
    cutoffs: List[int] = field(default_factory=lambda: [100,200])


@dataclass
class OutputConfig:
    save_dir: str = "figures_out"
    filename: str = "fig_early"
    fmt: str = "pdf"
    show: bool = True
...
def fig_main_plot(data, task_maps, data_cfg, fig_cfg, cutoff, save_path=None):
    ...
    for gi, group in enumerate(fig_cfg.groups):
        ...
        ax.set_title(f"{group.label}  (sub ≤ {cutoff})", fontsize=12, pad=6)
    ...
    for density in data_cfg.densities:
        ...
        for cutoff in data_cfg.cutoffs:
            ...
            def sp(tag):
                return os.path.join(
                    out_cfg.save_dir,
                    f"{out_cfg.filename}_{tag}_sub{cutoff}_density{density:.4f}.{out_cfg.fmt}"
                )
