...
A fractional level is plotted only where it gives a whole number of tasks; see
`resolve_level`.
...
def m_style(m_spec):
    """Linestyle is the simultaneity channel: dotted at the sequential limit,
        dashed at intermediate m, solid at the simultaneous limit.
    """
    if m_spec == 'T':
        return FL.LS_MT
    if m_spec == 1:
...
def level_int(m_spec, T: int) -> Optional[int]:
    """The whole number of tasks this level denotes at this T, or None where the
        fraction does not divide the repertoire.
    """
    if m_spec == 'T':
        return T
    if isinstance(m_spec, Fraction):
...
def resolve_level(m_spec, available, T: int):
    """Simultaneity level for one task number, or None if it is not defined
        there. A fraction is honoured only when it gives a whole number of tasks,
        since rounding would plot a different degree of simultaneity from the one
        the line represents.
    """
    if isinstance(m_spec, Fraction):
        product = m_spec * T
...
def make_figure(data, spec: FL.CacheSpec, cutoff: FL.Cutoff,
    ...
    # Colour means task divergence here as in Figures 2 and 3, even though each
    # column already fixes it.
    ...
    for r, (metric, ylabel) in enumerate(METRICS):
        for c, dT in enumerate(spec.task_divs):
            ...
            for i, m_spec in enumerate(m_specs):
                ...
                # alpha is lower than in the one-line-per-colour figures.
