#!/usr/bin/env python3
"""
figS_endpoint_metrics.py
========================
Endpoint (or fixed-cutoff) multi-metric summary across the m sweep.

Default behavior:
- One figure per density
- 3 rows x N columns, one column per T/K ratio
  Row 1: pheno_dist / mean pairwise task-target distance at cutoff
  Row 2: usage cosine dissimilarity at nearest snapshot <= cutoff
  Row 3: Jaccard distance between program gene sets at nearest snapshot <= cutoff
...
Pass --cutoff 200 (default) to evaluate at substitution step 200.
Pass --cutoff None (omit the flag and set default) to use the final state.
"""
...
class OutputConfig:
    ...
    filename: str = "figS_endpoint_metrics"
    fmt: str = "pdf"
    show: bool = True
...
def endpoint_pheno_dist_norm(reps: List[Dict], target_dist: float,
                             cutoff: Optional[int] = None) -> np.ndarray:
    """Normalized phenotype distance at cutoff (or final state if cutoff is None)."""
...
def _snapshot_at_cutoff(r: Dict, cutoff: Optional[int]) -> Dict:
    """Return snapshot at the step closest to but not exceeding cutoff-1.
    If cutoff is None, use the final snapshot."""
    snap_steps = sorted(r["snapshots"].keys())
    if cutoff is None:
        return r["snapshots"][snap_steps[-1]]
    eligible = [s for s in snap_steps if s <= cutoff - 1]
    step = eligible[-1] if eligible else snap_steps[0]
    return r["snapshots"][step]


def endpoint_usage_cosine(reps: List[Dict],
                          cutoff: Optional[int] = None) -> np.ndarray:
    """Usage cosine dissimilarity at nearest snapshot <= cutoff (or final)."""
    vals = []
    for r in reps:
        snap = _snapshot_at_cutoff(r, cutoff)
        vals.append(cosine_dissim_usage(snap["usage"]))
    return np.array(vals, dtype=float)


def endpoint_jaccard(reps: List[Dict],
                     cutoff: Optional[int] = None) -> np.ndarray:
    """Jaccard distance at nearest snapshot <= cutoff (or final)."""
    vals = []
    for r in reps:
        snap = _snapshot_at_cutoff(r, cutoff)
        vals.append(jaccard_dist(snap["genome"]))
    return np.array(vals, dtype=float)
...
def _get_endpoint_vals(reps, row_key, target_dist=None, cutoff=None):
    """Dispatch to the right metric extractor per row."""
    if row_key == "pheno_dist_norm":
        return endpoint_pheno_dist_norm(reps, target_dist, cutoff=cutoff)
    elif row_key == "usage_cosine":
        return endpoint_usage_cosine(reps, cutoff=cutoff)
    elif row_key == "jaccard":
        return endpoint_jaccard(reps, cutoff=cutoff)
    else:
...
def fig2_m_sweep(
    ...
    Metrics evaluated at data_cfg.cutoff (or final state if None).
    """
    cutoff = data_cfg.cutoff
    cutoff_label = f"sub ≤ {cutoff}" if cutoff is not None else "final"
...
def print_summary(
    ...
    """Print exact values for all three metrics at data_cfg.cutoff."""
    cutoff = data_cfg.cutoff
    cutoff_label = f"sub ≤ {cutoff}" if cutoff is not None else "final"
    ...
    print("\n" + "=" * 90)
    print(f"NUMERICAL SUMMARY — endpoint metrics ({cutoff_label})")
    print("=" * 90)
    header = (
        f"{'T/K':>5}  {'dT':>5}  {'m':>3}  "
        f"{'pheno':>8}  {'p_std':>7}  "
    ...
    for T in data_cfg.T_values:
        for dT in data_cfg.task_divs:
            ...
            for m in sorted(data[T][dT].keys()):
                ...
                pv = endpoint_pheno_dist_norm(reps, td, cutoff=cutoff)
                cv = endpoint_usage_cosine(reps, cutoff=cutoff)
                jv = endpoint_jaccard(reps, cutoff=cutoff)
...
def parse_args():
    ...
    p.add_argument("--filename", type=str, default="figS_endpoint_metrics")
    ...
    p.add_argument("--cutoff", type=int, default=defaults.cutoff)
